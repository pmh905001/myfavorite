...
def send_async_request(id, url, html_file_name):
    headers = {
    ...
    while response.is_redirect:
        ...
        response = requests.get(location, headers,allow_redirects=False)
    ...
    with open(html_file_name, 'a', encoding='utf-8') as file:
        record = json.dumps({id: response.text})
        file.write(record)
        file.write('\n')
...
def main():
    ...
    for id, url in id_urls:
        ...
        send_async_request(id, url, html_file_name)
